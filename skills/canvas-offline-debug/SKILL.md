---
name: canvas-offline-debug
description: >
  Debug Power Apps Canvas App offline / mobile behaviour. Diagnoses blank screens, silent ForAll
  failures, OData vs SQLite routing, ParseJSON crashes, nav-col access failures. Knows confirmed
  patterns for BESA_BookingElements, BESA_Envelopes, BESA_Tests, BESA_Shifts, BESA_TestRuns.
  Reads pa.yaml files to inspect current formulas but NEVER edits them — all fixes are described
  as Power Apps Studio formula edits for the user to apply manually. Tests hypotheses before
  assuming causes. Triggers on: offline, mobile blank, SQLite, OData, ForAll 0 rows, gallery blank,
  varActionRecord blank, nav col blank, Collect failing, ParseJSON crash, Kind:23, perm test screen,
  engineer app, BESA.
---
...
| `CountRows(Filter(BESA_BookingElements, ...))` | ✅ SQLite |
| `Concat(Filter(BESA_BookingElements, ...), ...)` | ✅ SQLite |
| `First(Filter(BESA_BookingElements, Text(BESA_BookingElement) = buf.bePK))` inside ForAll body | ✅ SQLite |

### BESA_Envelopes
| Operation | Result |
|-----------|--------|
| `LookUp(BESA_Envelopes, ...)` standalone | ❌ OData → blank always |
| `First(Filter(BESA_Envelopes, Text('Booking Element LookUp'.BESA_BookingElement) = buf.bePK))` in ForAll body | ✅ SQLite |
| `Concat(Filter(BESA_Envelopes, Text('Booking Element LookUp'.BESA_BookingElement) = bePK), Text('Floor Area'))` | ✅ SQLite |
...
| `Concat(Filter(BESA_Tests, ...), Text(BESA_Test) & ",")` | ✅ SQLite |
| `First(Filter(BESA_Tests, Text(BESA_Test) = g.testGUID))` in ForAll body | ✅ SQLite |

### BESA_Shifts
| Operation | Result |
|-----------|--------|
| `Filter(BESA_Shifts, 'User GUID' = x)` OData | ⚠️ Non-deterministically flaky (0 or correct) |
| `Filter(BESA_Shifts, 'Shift Date' >= d, ...)` OData | ⚠️ Flaky |
| `Filter(BESA_Shifts, Adhoc = true)` OData | ✅ Boolean column OData is reliable |
| `Concat(Filter(BESA_Shifts, 'User GUID' = x, 'Shift Date' >= d, ...), ...)` | ✅ SQLite |
| `First(Filter(BESA_Shifts, Text(BESA_Shift) = textGUID))` in ForAll body | ✅ SQLite |
